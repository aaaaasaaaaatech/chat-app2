...
RSpec.describe 'チャットルームの削除機能', type: :system do
  ...
  it 'チャットルームを削除すると、関連するメッセージがすべて削除されている' do
    ...
    # メッセージ情報を5つDBに追加する
    FactoryBot.create_list(:message, 5, room_id: @room_user.room.id, user_id: @room_user.user.id) #メッセージが削除されているか確認するためにcreate_listを用いて、メッセージと紐付いた、ユーザーとチャットルームを中間テーブルに生成
